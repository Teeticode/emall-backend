...
def login(user_cr: OAuth2PasswordRequestForm = Depends(), db: Session=Depends(database.get_db)):
    ...
    log_user = db.query(models.Developer).filter(models.Developer.email == user_cr.username).first()
    if not log_user:
        raise HTTPException(
            status_code= status.HTTP_403_FORBIDDEN,
            detail=f'Invalid Credentials')
    ...
    # create a token
    access_token = oauth2.create_access_token(data = {"user_id": log_user.devid})
    # return token
    return{'token':access_token, "token_type": "Bearer"}
